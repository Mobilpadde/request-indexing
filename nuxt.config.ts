...
let tokens: Partial<OAuthPoolToken>[] = env.NUXT_OAUTH_POOL
  ? JSON.parse(env.NUXT_OAUTH_POOL)
  : [];
const privateTokens: Partial<OAuthPoolToken>[] = env.NUXT_OAUTH_PRIVATE_POOL
  ? JSON.parse(env.NUXT_OAUTH_PRIVATE_POOL)
  : [];
  ...
  modules: [
    ...
    (_, nuxt) => {
      ...
      nuxt.options.nitro.virtual["#app/token-pool.mjs"] = [
        ...
        `export const privateTokens = ${JSON.stringify(
          privateTokens.map((t) => {
            t.id = t.id || hash(t);
            return t;
          })
